...
import AbstractChart, { type AbstractChartConfig, type AbstractChartProps, DEFAULT_X_LABELS_HEIGHT_PERCENTAGE } from './abstract-chart';
...
class StackedBarChart extends AbstractChart<StackedBarChartProps, StackedBarChartState> {
	getBarPercentage = () => {
	...
	renderBars = ({
		...
		'width' | 'height' | 'paddingRight' | 'paddingTop' | 'stackedBar' | 'verticalLabelsHeightPercentage'
	> & {
		border: number;
		colors: string[];
		data: number[][];
		...
		legend.map((x, i) => {
			return (
				<G key={Math.random()}>
					...
					<Text x={width * 0.78} y={height * 0.76 - i * 50} {...this.getPropsForLabels()}>
						{x}
					</Text>
				</G>
			);
