...
import AbstractChart, { type AbstractChartProps } from './abstract-chart';
...
class PieChart extends AbstractChart<PieChartProps, PieChartState> {
	render() {
		const { style = {}, backgroundColor, absolute = false, hasLegend = true, avoidFalseZero = false } = this.props;
		...
		const slices = chart.curves.map((c, i) => {
			...
			if (absolute) {
				...
				const percentage = Math.round((100 / total) * c.item[this.props.accessor]);
		...
		return (
			<View
				...
				<Svg width={this.props.width} height={this.props.height}>
					...
					<G x={this.props.width / 2 / 2 + Number(this.props.paddingLeft ? this.props.paddingLeft : 0)} y={this.props.height / 2}>
						{slices}
					</G>
				</Svg>
			</View>
