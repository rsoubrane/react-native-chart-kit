...
import { Animated, ScrollView, StyleSheet, TextInput, View, type ViewStyle } from 'react-native';
...
import type { ChartData, Dataset } from '../helper-types';
...
class LineChart extends AbstractChart<LineChartProps, LineChartState> {
	...
	renderDots = ({
		...
		paddingRight,
		onDataPointClick
	}: Pick<AbstractChartConfig, 'data' | 'width' | 'height' | 'paddingRight' | 'paddingTop'> & {
		...
		data.forEach((dataset) => {
			...
			dataset.data.forEach((x, i) => {
				...
				const cy = ((baseHeight - this.calcHeight(x, datas, height)) / 4) * 3 + paddingTop;
				...
				output.push(
					...
					/>,
					<Circle key={Math.random()} cx={cx} cy={cy} r="14" fill="#fff" fillOpacity={0} onPress={onPress} />,
	...
	renderScrollableDot = ({
		...
		data.forEach((dataset) => {
			...
			for (let index = 0; index < dataset.data.length; index++) {
				...
				const yval =
					((baseHeight - this.calcHeight(dataset.data[dataset.data.length - index - 1], datas, height)) / 4) * 3 + paddingTop;
				yValues.push(yval);
				const xval = paddingRight + ((dataset.data.length - index - 1) * (width - paddingRight)) / dataset.data.length;
				xValues.push(xval);
			...
			return (
				<Polygon
					...
					points={`${dataset.data
						...
						.join(' ')} ${paddingRight + ((width - paddingRight) / dataset.data.length) * (dataset.data.length - 1)},${
	...
	renderLine = ({
		...
		data,
		linejoinType
	}: Pick<AbstractChartConfig, 'data' | 'width' | 'height' | 'paddingRight' | 'paddingTop' | 'linejoinType'>) => {
		if (this.props.bezier) {
	...
	getBezierLinePoints = (
		dataset: Dataset,
		{
			...
			paddingTop,
			data
		}: Pick<AbstractChartConfig, 'width' | 'height' | 'paddingRight' | 'paddingTop' | 'data'>
	) => {
		...
		const x = (i: number) => Math.floor(paddingRight + (i * (width - paddingRight)) / xMax);
	...
	renderBezierShadow = ({
		...
		data.map((dataset, index) => {
			...
			const d = `${this.getBezierLinePoints(dataset, {
				...
				paddingTop,
				data
			})} L${paddingRight + ((width - paddingRight) / xMax) * (dataset.data.length - 1)},${
				(height / 4) * 3 + paddingTop
			} L${paddingRight},${(height / 4) * 3 + paddingTop} Z`;

			return (
				<Path key={index} d={d} fill={`url(#fillShadowGradientFrom${useColorFromDataset ? `_${index}` : ''})`} strokeWidth={0} />
			);
		});
